...
pub fn to_ass(captions: &srv3_ttml::TimedText) -> std::io::Result<String> {
    ...
    const MARGIN_L: &str = "0";
    const MARGIN_R: &str = "0";
    const MARGIN_V: &str = "0";
    const LAYER: &str = "0";
    ...
    // Build the ASS document content as a string that can be parsed
    let mut w = String::new();
    ...
    // Define all the standard YouTube styles
    let style_lines = [
        "Style: YTPlain,Roboto,38,&H01FEFEFE,&HFF000000,&H00000000,&H00000000,0,0,0,0,100,100,0,0,1,0,0,2,25,25,15,1",
        ...
        "Style: YTBevelBox,Roboto,38,&H01FEFEFE,&HFF000000,&H01000000,&H00000000,0,0,0,0,100,100,0,0,3,0.01,4,2,25,25,15,1",
    ];
    
    for style_line in &style_lines {
        writeln!(&mut w, "{}", style_line).unwrap();
    }
    w.push('\n');
    ...
    for element in &captions.body.elements {
        if let BodyElement::Paragraph(paragraph) = element {
            ...
            let mut style_name: &str = DEFAULT_STYLE;
    ...
    // Parse and re-serialize using ass-core for validation and normalization
    match Script::parse(&w) {
        Ok(script) => {
            // Use ass-core's serialization
            Ok(script.to_ass_string())
        }
        Err(_) => {
            // If parsing fails, return the manually built string
