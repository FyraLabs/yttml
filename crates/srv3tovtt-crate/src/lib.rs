...
pub fn to_ass(captions: &srv3_ttml::TimedText) -> std::io::Result<String> {
    ...
    // Create a basic ASS document with required sections
    let initial_content = format!(
        "[Script Info]\n\
         ; Script generated by YTTML\n\
         ; https://github.com/FyraLabs/yttml/\n\
         ScriptType: v4.00+\n\
         WrapStyle: 0\n\
    ...
    for element in &captions.body.elements {
        if let BodyElement::Paragraph(paragraph) = element {
            ...
            // Use EventBuilder to create the dialogue line
